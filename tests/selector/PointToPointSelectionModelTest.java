...
class PointToPointSelectionModelTest {
    ...
    void testLiveWireNonEmpty() {
        ...
        PolyLine wire = model.liveWire(endpoint);

        // Verify the consequences
        PolyLine expectedWire = new PolyLine(startPoint, endpoint);
        assertEquals(expectedWire, wire);
    }

    @DisplayName("GIVEN a non-empty selection model, WHEN undo is called, THEN the expected state"
            + "becomes the SELECTING state, it gives a notification of the absense of state, "
            + "it asserts selection size is reduced by one, there is notification of selection "
            + "change, and the expected last point becomes the previous penultimate point ")
    @Test
    void testUndoNonEmpty() {
        SelectionModel segment = new PointToPointSelectionModel(false);
        segment.addPoint(new Point(0,0));

        PclTester observer = new PclTester();
        segment.addPropertyChangeListener(observer);

        segment.undo();

        observer.assertChangedTo("state", NO_SELECTION);
        assertEquals(NO_SELECTION,segment.state());

        assertTrue(segment.selection().isEmpty());
    }
    ...
    @DisplayName("GIVEN a selection, WHEN the starting point of the selection is moved, THEN"
            + "the first segment's start and the last segment's end are moved to the new location")
    @Test

    void testMovePointStart() {
        SelectionModel model = makeSquareSelection();
        PclTester pclTester = new PclTester();
        model.addPropertyChangeListener(pclTester);

        model.movePoint(0, new Point(1,2));
        pclTester.assertChanged("selection");

        PolyLine seg1 = model.selection().get(0);
        PolyLine seg2 = model.selection().getLast();

        assertEquals(new Point(1,2), seg1.start());
        assertEquals(new Point(1,2), seg2.end());
    ...
    @DisplayName("Given a selection, WHEN querying for the closest location that is within the "
            + "maximum distance of an unambiguously closest point but not directly on top of it")
    @Test
    void testClosestPointWithin() {
        SelectionModel model = makeSquareSelection();
        assertEquals (0, model.closestPoint (new Point(1, 1),  9));
        assertEquals(1, model.closestPoint (new Point(11, 1), 9));
        assertEquals( 2, model.closestPoint (new Point (11, 11),9));
        assertEquals( 3, model.closestPoint (new Point( 1, 12) , 9));
    }
